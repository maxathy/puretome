...
exports.createMemoir = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error creating memoir', error: err.message });
  }
};
...
exports.deleteMemoir = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error removing memoir', error: err.message });
  }
};
...
exports.getMemoirById = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error retrieving memoir', error: err.message });
  }
};
...
exports.getMyMemoirs = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error retrieving memoirs', error: err.message });
  }
};
...
exports.respondToInvitation = async (req, res) => {
  try {
    ...
    const alreadyCollaborator = memoir.collaborators.some(
      (c) =>
        c.inviteEmail?.toLowerCase() === invitation.inviteeEmail.toLowerCase() ||
        (inviteeUser && c.user && c.user.toString() === inviteeUser._id.toString()) // Check if user ID matches
    );
